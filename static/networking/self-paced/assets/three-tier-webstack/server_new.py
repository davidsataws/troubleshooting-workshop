...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            mdtest, md_time, metadata = get_metadata(False)
            extservertest, ext_time = call_extServer()
            dnstest, dns_time = call_DNS()
...
def call_CANARY(region):
    ...
    try:
        ...
        synthetics_client = boto3.client('synthetics', region_name="us-east-1")
        response = synthetics_client.describe_canaries_last_run(
            Names=[canary_name]
        )
        # Extract the last run information
        if response and 'CanariesLastRun' in response and len(response['CanariesLastRun']) > 0:
            last_run_info = response['CanariesLastRun'][0]['LastRun']

            # Get the status of the last run
            last_run_state = last_run_info['Status']['State']
            last_run_reason = last_run_info['Status'].get('StateReason', 'N/A')
            if last_run_state == 'PASSED':
                result = "SUCCESS"
        else:
            print(f"No last run information found for canary: {canary_name}")
    except Exception as e:
        logging.warning('Call to Synthetics failed.')
        logging.warning(e)

    end_time = datetime.now()
    syntheticstime =  (end_time - start_time)
    return result, round(syntheticstime.total_seconds() * 1000,2)

def call_ADMIN(region):
    start_time = datetime.now()
    elb_name = 'WebApp-ALB'
    result = "FAILED"
